...
<?php
	...
	//$citysite = "Chandler";
		...
		$LoginRS__query=sprintf("SELECT id_city, user_admin, password FROM master_admin WHERE user_admin=%s AND password=%s AND city_name = '$citysite'",
			GetSQLValueString($loginUsername, "text"), GetSQLValueString($password, "text")); 
   
		$LoginRS = mysqli_query($LoginRS__query, $admin_buy_local) or die(mysqli_error());
		...
		if($row_citymstr = mysqli_fetch_array($LoginRS)){
			$citymstr = $row_citymstr['id_city'];
		}
		if ($loginFoundUser) {
			$loginStrGroup = "";
